number_plates: list = ['HH9901CC', 'CA9150WC', 'BH7144WB', 'AI7665KU', 'AI8045DS', 'HH1233HH', 'AI1552MG', 'AI7232BO',
...
input_number = input("Enter the number in the format XX1111XX: ")
...
validation_result = validation(input_number)

while not validation_result:
    input_number = input("Number entered wrong. Enter the number in the format XX1111XX: ")
    input_number = input_number.upper()
    validation_result = validation(input_number)
# ==== 2 ====
unique_number_plates = set(number_plates)
print('-> Unique numbers:', len(unique_number_plates))

# ==== 3 ====
if input_number in number_plates:
    print('-> Entered number is on the list')
else:
    print('-> Your number is not on the list')

# ==== 4 ====
def parse_input_number_with_loop(number):
    temp = ""
    transform_result = []
    ...
    for character in number:
        if not character.isdigit():
            temp += character
            if len(temp) == 2:
                transform_result.append(temp)
                temp = ""
        else:
            transform_result.append(int(character))

    return transform_result

# ==== 5 ====
def sum_of_the_numbers(array_of_chunks):
    sum_of_numbers = 0

    for chunk in array_of_chunks:
        if not type(chunk) == str:
            sum_of_numbers += chunk
...
parsed_array = parse_input_number_with_loop(input_number)
print(f"-> Parsed input number: {parsed_array}")

sum_result = sum_of_the_numbers(parsed_array)
print(f"-> Sum of numbers in your input number = {sum_result}")